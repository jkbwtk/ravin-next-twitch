...
export const channelActionsTable = pgTable('ChannelActions', {
  ...
  createdAt: timestamp('createdAt', { precision: 3, mode: 'string' }).defaultNow().notNull(),
  updatedAt: timestamp('updatedAt', { precision: 3, mode: 'string' }).defaultNow().notNull(),
  channelUserId: varchar('channelUserId').notNull().references(() => usersTable.id, { onDelete: 'cascade' }),
},
(table) => {
  return {
...
export const commandsTable = pgTable('Commands', {
  ...
  updatedAt: timestamp('updatedAt', { precision: 3, mode: 'string' }).defaultNow().notNull(),
  channelUserId: varchar('channelUserId').notNull().references(() => usersTable.id, { onDelete: 'cascade' }),
  templateId: integer('templateId').notNull().references(() => templatesTable.id),
},
(table) => {
  return {
...
export const behaviorProfilesToCommandsTable = pgTable('BehaviorProfilesToCommands', {
  ...
  B: integer('B').notNull().references(() => commandsTable.id, { onDelete: 'cascade', onUpdate: 'cascade' }),
},
(table) => {
  return {
...
export const behaviorProfilesToPhraseFiltersTable = pgTable('BehaviorProfilesToPhraseFilters', {
  A: integer('A').notNull().references(() => behaviorProfilesTable.id, { onDelete: 'cascade', onUpdate: 'cascade' }),
  B: integer('B').notNull().references(() => phraseFiltersTable.id, { onDelete: 'cascade', onUpdate: 'cascade' }),
},
(table) => {
  return {
...
export const behaviorProfilesToRegexFiltersTable = pgTable('BehaviorProfilesToRegexFilters', {
  A: integer('A').notNull().references(() => behaviorProfilesTable.id, { onDelete: 'cascade', onUpdate: 'cascade' }),
  B: integer('B').notNull().references(() => regexFiltersTable.id, { onDelete: 'cascade', onUpdate: 'cascade' }),
},
(table) => {
  return {
...
export const behaviorProfilesToCommandTimersTable = pgTable('BehaviorProfilesToCommandTimers', {
  A: integer('A').notNull().references(() => behaviorProfilesTable.id, { onDelete: 'cascade', onUpdate: 'cascade' }),
  B: integer('B').notNull().references(() => commandTimersTable.id, { onDelete: 'cascade', onUpdate: 'cascade' }),
},
(table) => {
  return {
...
export const botActionsTable = pgTable('BotActions', {
  id: serial('id').primaryKey().notNull(),
